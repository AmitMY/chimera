...
CorpusPreProcessPipeline.enqueue("corpus", "Read Corpus", lambda f, _: WebNLGDataReader(f["set"]))
...
CorpusPreProcessPipeline.enqueue("tokenize", "Tokenize Plans & Sentences",
                                 lambda f, _: f["match-plans"].copy().tokenize())
...
TestCorpusPreProcessPipeline.enqueue("tokenize", "Tokenize Plans & Sentences", lambda f, _: f["plan"].copy().tokenize())
